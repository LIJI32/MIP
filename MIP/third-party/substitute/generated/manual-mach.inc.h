...
static
kern_return_t manual_mach_vm_remap_new
(
	vm_map_t target_task,
	mach_vm_address_t *target_address,
	mach_vm_size_t size,
...
#pragma pack(push, 4)
	...
	typedef struct {
		...
		vm_prot_t max_protection;
		vm_inherit_t inheritance;
	} __Request__manual_mach_vm_remap_new_t __attribute__((unused));
#ifdef  __MigPackStructs
#pragma pack(pop)
#endif
#ifdef  __MigPackStructs
...
#pragma pack(push, 4)
	...
	typedef struct {
		...
		vm_prot_t cur_protection;
		vm_prot_t max_protection;
	} __Reply__manual_mach_vm_remap_new_t __attribute__((unused));
#ifdef  __MigPackStructs
#pragma pack(pop)
#endif
#ifndef	LimitCheck
...
#pragma pack(pop)
	...
	msg_result = manual_mach_msg(&InP->Head, 0x200000000|MACH_SEND_MSG|MACH_RCV_MSG|MACH_MSG_OPTION_NONE, (mach_msg_size_t)sizeof(Request), (mach_msg_size_t)sizeof(Reply), InP->Head.msgh_reply_port, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
...
static kern_return_t __MIG_check__Reply__manual_mach_vm_remap_new_t(__Reply__manual_mach_vm_remap_new_t *Out0P)
{

	typedef __Reply__manual_mach_vm_remap_new_t __Reply __attribute__((unused));
	unsigned int msgh_size;
	if (Out0P->Head.msgh_id != 4921) {
	    if (Out0P->Head.msgh_id == MACH_NOTIFY_SEND_ONCE)
		{ return MIG_SERVER_DIED; }
	    else
		{ return MIG_REPLY_MISMATCH; }
...
/* Routine mach_vm_remap_new */
static kern_return_t manual_mach_vm_remap_new
(
	vm_map_t target_task,
	mach_vm_address_t *target_address,
	mach_vm_size_t size,
...
#pragma pack(pop)
	...
	__DeclareSendRpc(4821, "mach_vm_remap_new")
	...
	/* msgh_size passed as argument */
	InP->Head.msgh_request_port = target_task;
	InP->Head.msgh_id = 4821;
	InP->Head.msgh_reserved = 0;



	__BeforeSendRpc(4821, "mach_vm_remap_new")
	msg_result = manual_mach_msg(&InP->Head, MACH_SEND_MSG|MACH_RCV_MSG|MACH_MSG_OPTION_NONE, (mach_msg_size_t)sizeof(Request), (mach_msg_size_t)sizeof(Reply), InP->Head.msgh_reply_port, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
	__AfterSendRpc(4821, "mach_vm_remap_new")
	if (msg_result != MACH_MSG_SUCCESS) {
		__MachMsgErrorWithoutTimeout(msg_result);
	}
	if (msg_result != MACH_MSG_SUCCESS) {
	...
	check_result = __MIG_check__Reply__manual_mach_vm_remap_new_t((__Reply__manual_mach_vm_remap_new_t *)Out0P);
	if (check_result != MACH_MSG_SUCCESS) {
		mach_msg_destroy(&Out0P->Head);
		{ return check_result; }
	}
